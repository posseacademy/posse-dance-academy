...
## 2026-09-03: レッスンに開催期間を持たせ、月をまたぐ差し替えを可能にする

**ユーザー意図 (User Intent)**: 「9月からの使用を変える」— 水曜天神の「ブレイキン中級 HARUHIKO」を新クラス（breakin' バトルトレーニング / Key-lock）に差し替え、火曜大橋に新クラス（breakin' ミュージカリティ&スタイル / SHIN）を追加する。条件は「**8月以前のデータはそのまま残す。9月以降から新規に変える**」。さらに「タイムスケジュールのレッスン追加から私が入力するので、そこから入力した情報がすべてに反映するようにします。よって、あなたがクラスを追加する必要はありません。**そのシステムを作るのが重要です**」。つまり依頼はクラス登録の代行ではなく、**月境界で差し替えられる仕組みの実装**。登録操作はユーザーが画面から行う。

**前提の確認**: 2026-08-10 の連動修正（`app.js?v=119`）は本番反映済みで、残っていたのは受け入れテストのみだった。この経緯はセッション履歴 `98147b31-...jsonl`（8/10・6.7MB）に残っていた。`search_session_transcripts` は posse のセッションをインデックスしておらず「該当なし」を返すため、**`~/.claude/projects/<project>/*.jsonl` を直接 grep する**のが確実。

**Decision**:
1. 時間割エントリに `activeFrom` / `activeThrough`（ともに `YYYY-MM`・**包含**）を追加。`isLessonActiveIn()` が判定する。フォールバックは「両方無ければ常時開催」で既存34エントリは無変更。
2. `isClassInTimeSchedule(cls, dayLessons, month)` に第3引数を追加。表示（週間・月間・出席名簿・HOME・CSV）はすべてこれを通す。月間スケジュールだけは `app.selectedMonth` ではなく**その日が属する月**で判定する。
3. `isClassEnded()` を新設。「最終開催月を過ぎたクラスは、名簿に生徒が残っていても隠す」。これが無いと `getClassStudentsForMonth().total > 0` の既存条件に拾われ、差し替えのたびに生徒ひとりずつへ `leftAt` を入れて回る必要が出る。
4. **`isClassEnded` は終了側（`activeThrough`）しか見ない。** 開始側まで見ると、時間割から消えたクラスを登録し直したとき（新規フォームの開始月は当月が既定なので必発）に**そのクラスの過去の出席記録が出席名簿から丸ごと消える**。2026-08-10 Step 3 の「ブレイキン入門 AYANO / HARUHIKO を復旧」が実際にこの経路だった。開始前の月は従来どおり受講者数に委ねる（新設クラスに過去の受講者はいないので過去月には出ない）。
5. `ensureMonthlyPlanSnapshot()` にも同じ `isClassEnded` フィルタを入れる。
6. 週間時間割に月ナビゲーションと「この月に開催されないレッスンは表示していません」バナーを追加。

**Reason**: `schedule`（名簿）も `timeSchedule`（時間割）も**全月共通**の1ドキュメントで、月別に分かれていない。そのため新クラスを1つ足すと過去すべての月の出席名簿に0名のカードが出る。2026-08-10 に「月フィルタは実装しない」と決めていたが、今回「8月以前はそのまま／9月から新規」という要求が出たことで前提が変わった。データ構造を月別に分割するのは影響が大きすぎるため、**時間割エントリ側に期間を持たせて表示で出し分ける**方式を採った。

**Impact**: `new-app/js/utils.js`（+3関数）, `views/{schedule,attendance,home}.js`, `csv-export.js`, `app.js`, `app.html`, `CLAUDE.md`, `references/レッスン開催期間の運用.md`（新規）。`app.js?v=120` / `utils.js?v=19` / `schedule.js?v=28` / `attendance.js?v=52` / `home.js?v=30` / `customers.js?v=21` / `csv-export.js?v=22`。**Firestore のデータは一切変更していない**（クラス登録はユーザーが画面から行うため）。

**検証**: 単体テスト28件（期間判定・差し替えシナリオ・再登録時の過去記録保全・後方互換・年またぎ・alias 同期漏れ）、全7ファイルの構文チェック、ローカルサーバ（`.claude/launch.json` の `posse-app`）での実機確認。実機では Firestore に書かず**メモリ上の `timeScheduleData` だけを書き換えて**、9月に旧クラスが消え新クラスが出ること・8月に戻すと旧クラスが復活すること・出席名簿でも同じ挙動になることを目視確認した。

**Pattern**: failure → success — **表示フィルタを足したら、同じ条件が Firestore の書き込み側にも要るかを必ず確かめる**。`isClassEnded` で出席名簿から隠したクラスに対し、`ensureMonthlyPlanSnapshot()` が新月アクセス時に `attendance_YYYYMM` のキーを作り続けていた（`@agent-code-reviewer` が指摘）。それを `getClassStudentsForMonth` の attendance スキャンが `pastRegulars` として拾い直すため、隠したはずのクラスが幽霊として復活する。2026-04 の `migrateOrphanRegulars()`（退会者の自動復活）と同型の事故で、**「隠す」は「書かない」を意味しない**。もう1つ、**表示を月でフィルタする画面には、その月を表示し、移動する手段を必ず置く** — 週間時間割は `app.selectedMonth`（出席名簿タブでしか変えられない値）で暗黙に絞られており、消えたレッスンを編集・削除する経路がユーザーから見えなくなっていた。
