...
import { renderCustomers, renderAddForm, renderCustomerRow } from './views/customers.js?v=4';
import { renderAttendance, renderAttendanceOverview, renderAttendanceRecord, renderPracticeSession, renderAddStudentForm, renderEventRecord } from './views/attendance.js?v=11';
...
class DanceStudioApp {
    ...
    async init() {
        ...
        this.eventAttendanceData = await db.loadEventAttendance(this.selectedMonth);
    ...
    // ===== EVENT MANAGEMENT =====
    addEventParticipant() {
        this.eventAttendanceData.push({ id: null, isMember: false, lastName: '', firstName: '', week1: '×', week2: '×', week3: '×', fee: 0, memo: '' });
        this.render();
    }
    ...
    async deleteEventParticipant(index) {
        ...
        if (participant.id) {
            try { await db.deleteEventParticipantDoc(this.selectedMonth, participant.id); } catch (error) { console.error('削除エラー:', error); }
        }
        this.eventAttendanceData.splice(index, 1);
        await db.saveEventAttendance(this.selectedMonth, this.eventAttendanceData);
        this.eventAttendanceData = await db.loadEventAttendance(this.selectedMonth);
        this.render();
    }
    ...
    async saveNewClass() {
        ...
        this.scheduleData['イベント'].push({ location, name: className, color, students: [] });
        await db.saveScheduleData(this.scheduleData);
    ...
    setupAttendanceEvents() {
        ...
        // Event attendance
        document.getElementById('addEventParticipantBtn')?.addEventListener('click', () => this.addEventParticipant());
        document.querySelectorAll('.event-attendance-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                this.eventAttendanceData[index][week] = current === '○' ? '×' : '○';
                db.saveEventAttendance(this.selectedMonth, this.eventAttendanceData);
                this.render();
            });
        });
        document.querySelectorAll('[data-event-field]').forEach(input => {
            input.addEventListener('change', (e) => {
                ...
                if (field === 'fee') value = parseFloat(value) || 0;
            ...
            if (input.getAttribute('data-event-field') === 'fee') {
                input.addEventListener('input', (e) => {
                    const index = parseInt(e.target.getAttribute('data-event-index'));
                    this.eventAttendanceData[index].fee = parseFloat(e.target.value) || 0;
                    this.render();
                });
            }
        });
        document.querySelectorAll('.delete-event-participant-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                this.deleteEventParticipant(parseInt(btn.getAttribute('data-delete-event-index')));
            });
        });
